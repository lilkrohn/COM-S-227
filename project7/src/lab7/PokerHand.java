...
public class PokerHand
{
  ...
  sortNoSuit();
	Card lowerCard = cards[0];
	Card upperCard = cards[1];

	for(int i = 0; i < 4; i++) {
		lowerCard = cards[i];
		upperCard = cards[i+1];
		if(upperCard.getRank() != lowerCard.getRank() + 1) {
			return false;
		}
	}
  ...
  sortNoSuit();
	...
	if((card1.getRank() == card2.getRank()) && (card4.getRank() == card5.getRank()) && (card1.getRank() != card5.getRank())) {
		if(card1.getRank() == card3.getRank()) {
			return true;
		}
		else if (card5.getRank() == card3.getRank()){
			return true;
		}
	}
	return false;
